...
def training(dataset, opt, pipe, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration <= opt.rvq_iter:
            ...
            render_pkg = render(viewpoint_cam, gaussians, pipe, background, image_shape=gt_image.shape, itr=iteration, rvq_iter=True)
        ...
        # loss = ((1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)))+ render_pkg["vqloss"]
        ...
        rendered_depth = render_pkg["depth"][0].cuda()
        # print(rendered_depth.shape)
        midas_depth = torch.tensor(viewpoint_cam.depth_image, dtype=torch.float32).cuda()
        # print(midas_depth.shape)
        
        # 获取rendered_depth的空间维度并调整midas_depth的分辨率
        # 获取 rendered_depth 的空间尺寸
        H, W = rendered_depth.shape
        # 首先，为 midas_depth 增加一个批次和一个通道维度以适配 interpolate 函数的输入格式
        midas_depth = F.interpolate(midas_depth.unsqueeze(0).unsqueeze(0), size=(H, W), mode='bilinear', align_corners=False).squeeze()
        ...
        if iteration % args.sample_pseudo_interval == 0 and iteration > args.start_sample_pseudo and iteration < args.end_sample_pseudo:
            ...
            render_pkg_pseudo = render(pseudo_cam, gaussians, pipe, background, image_shape=gt_image.shape, itr=iteration, rvq_iter=False)
        ...
        with torch.no_grad():
            ...
            if iteration == opt.iterations:
                ...
                # gaussians.final_prune(iteration)
            ...
            # else:
            #     if iteration > 2000 and iteration % opt.mask_prune_iter == 0:
            #         gaussians.mask_prune(iteration)
                # if iteration % opt.dist_prune_iter == 0:
                #     gaussians.dist_prune()
